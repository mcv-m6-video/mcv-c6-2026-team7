"""
UNet variant with dilated convolutions in the encoder and a multi-scale dilated
bottleneck (ASPP-style). Dilation grows with depth (1 → 2 → 4) so deeper
encoder stages capture larger temporal contexts without extra pooling.
"""
...
class TemporalUNetDilated(nn.Module):
    def __init__(self, channels, hidden_channels=128, out_channels=64):
        super().__init__()
        ...
        self.bottleneck = DilatedBottleneck(hidden_channels * 4)

...
class TemporalUNet(nn.Module):
    def __init__(self, channels, hidden_channels=256, out_channels=128, dropout_p=0.0):
        ...
        self.up3 = nn.ConvTranspose1d(hidden_channels * 4, hidden_channels * 4, kernel_size=2, stride=2)
        self.up2 = nn.ConvTranspose1d(hidden_channels * 2, hidden_channels * 2, kernel_size=2, stride=2)
        self.up1 = nn.ConvTranspose1d(hidden_channels, hidden_channels, kernel_size=2, stride=2)
    ...
    def forward(self, x):
        ...
        # Decoder
        d3 = self.up3(b, output_size=e3.size())
        d3 = torch.cat([d3, e3], dim=1) 
        d3 = F.relu(self.dec3(d3))
        d3 = self.dropout(d3)

        d2 = self.up2(d3, output_size=e2.size())
        d2 = torch.cat([d2, e2], dim=1) 
        d2 = F.relu(self.dec2(d2))
        d2 = self.dropout(d2)

        d1 = self.up1(d2, output_size=e1.size())

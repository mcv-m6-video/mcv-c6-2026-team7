...
OUTPUT_VIDEO = Path(__file__).parent / "results_yolo.mp4"

# Ground truth filtering options
GT_EXCLUDE_OUTSIDE  = True
GT_EXCLUDE_PARKED   = False
GT_EXCLUDE_OCCLUDED = False
GT_EXCLUDE_BIKES    = True

# Scale factor applied to both GT and predictions (1 = full resolution)
SCALE = 1
...
def main():
    ...
    # Class mapping
    label_to_class = {'car': 0}
    if not GT_EXCLUDE_BIKES:
        label_to_class['bike'] = 0
    ...
    for frame_idx in range(warmup_end, total_frames):
        ...
        gt_boxes = []
        for box in boxes:
            if GT_EXCLUDE_OUTSIDE and box.outside != 0:
                continue
            if box.label not in label_to_class:
                continue
            if GT_EXCLUDE_PARKED and box.attributes.get('parked') == 'true':
                continue
            if GT_EXCLUDE_OCCLUDED and box.occluded != 0:
                continue
    ...
    with open(DETECTIONS_FILE, 'r') as f:
        ...
        for line in f:
            ...
            if class_id != 2:  # only cars (COCO class 2)
                continue
            ...
            predictions[frame_id].append(((x, y, w, h), conf, 0))
    ...
    result = compute_map(
        ...
        num_classes=1,
        iou_threshold=0.5,
        replace_confidence_at_random=False,
        N=1,
    )

...
class Model(BaseRGBModel):
    ...
    class Impl(nn.Module):
        ...
        def __init__(self, args = None):
            ...
            if self._feature_arch.startswith(('rny002', 'rny004', 'rny008')):
            ...
            elif self._feature_arch.startswith('convnext'):
                features = timm.create_model(
                    self._feature_arch,
                    pretrained=True,
                    num_classes=0         # returns (B, D) pooled features directly
                )
                ...
                # Check how many stages to unfreeze (default is 0: fully frozen)
                unfreeze_last_n = getattr(args, 'unfreeze_stages', 0)
                
                if unfreeze_last_n > 0:
                    # Unfreeze the final head/norm layer since it directly processes the last stage's output
                    if hasattr(features, 'head'):
                        for param in features.head.parameters():
                            param.requires_grad = True
                    elif hasattr(features, 'norm'):
                        for param in features.norm.parameters():
                            param.requires_grad = True

                    # Unfreeze the requested number of last stages
                    total_stages = len(features.stages)
                    start_stage = max(0, total_stages - unfreeze_last_n)
                    
                    for i in range(start_stage, total_stages):
                        for param in features.stages[i].parameters():
                            param.requires_grad = True

                # Check the actual output dim
                with torch.no_grad():
                    dummy = torch.zeros(1, 3, 224, 224)
                    self._d = features(dummy).shape[-1]

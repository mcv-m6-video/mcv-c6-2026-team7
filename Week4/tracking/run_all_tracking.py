...
def dataset_det_to_internal_csv(det_txt: Path, out_csv: Path) -> None:
    """Convert dataset MOT-format det.txt to our internal detections CSV."""
    rows = []
    with open(det_txt) as fh:
        for line in fh:
            ...
            conf = float(parts[6])
            rows.append({"frame_id": frame, "x1": x, "y1": y,
                         "x2": x + w, "y2": y + h, "confidence": conf})
...
def run_tracking_method_inprocess(
    ...
    try:
        ...
        # copy tracks.txt up into output_dir so eval can find it at a stable path
        src_tracks = Path(exp_dir) / "tracks.txt"
        dst_tracks = output_dir / "tracks.txt"
        if src_tracks.exists() and src_tracks != dst_tracks:
            shutil.copy(src_tracks, dst_tracks)
...
def run_all_cameras(
    ...
    for cam_path, seq_name, cam_name in cameras:
        ...
        det_dir    = cam_path / detections_subpath
        ...
        # ── build detection sources ──────────────────────────────────────────
        det_sources = []
        if det_dir.is_dir():
            for det_file in sorted(det_dir.glob("det_*.txt")):
                model_label   = det_file.stem[len("det_"):]
            ...
            for model_label, _ in det_sources:
                ...
                if model_results:
                    make_comparison_plot(
                        ...
                        output_path=cam_out_dir / f"comparison_{model_label}.png",
                    )
...
def main(args=None) -> None:
    ...
    run_all_cameras(
        ...
        detections_subpath  = args.detections_subpath,

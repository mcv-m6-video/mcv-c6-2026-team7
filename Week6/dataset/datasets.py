...
def get_datasets(args):
    ...
    train_map_data = None
    val_map_data = None
    if getattr(args, 'map_train_val_eval_interval', 0) > 0:
        dataset_kwargs_map = dict(dataset_kwargs)
        dataset_kwargs_map['overlap'] = 0

        train_map_data = ActionSpotVideoDataset(
            classes, os.path.join('data', args.dataset, 'train.json'),
            args.frame_dir, args.clip_len, **dataset_kwargs_map)
        train_map_data.print_info()

        val_map_data = ActionSpotVideoDataset(
            classes, os.path.join('data', args.dataset, 'val.json'),
            args.frame_dir, args.clip_len, **dataset_kwargs_map)
        val_map_data.print_info()
    ...
    return classes, train_data, val_data, test_data, train_map_data, val_map_data

...
def compute_optical_flow(model: str, params: list, img1: np.ndarray, img2: np.ndarray) -> np.ndarray:
    ...
    if model == "pyflow":
        ...
        # Load the RAFT model (cached to avoid reloading every call)
        if model not in _model_cache:
            if model == "raft_large":
                _model_cache[model] = raft_large(weights=Raft_Large_Weights.DEFAULT, progress=False).to(device).eval()
            elif model == "raft_small":
                _model_cache[model] = raft_small(weights=Raft_Small_Weights.DEFAULT, progress=False).to(device).eval()
        raft_model = _model_cache[model]
        ...
        list_of_flows = raft_model(img1.to(device), img2.to(device))
